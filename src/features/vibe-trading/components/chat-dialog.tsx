...
import { ChatChipGroup } from "./chat-chip-group";
...
export function ChatDialog({
  ...
  const { messages, streaming, error, send, cancel } = useChatStream(
    sessionId,
    title,
  );
  const {
  ...
  // 的限制。
  const [pendingGoalObjective, setPendingGoalObjective] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    ...
    const hasRealSwarm = slot?.messages.some(
      (m) =>
        m.type === "swarm_status" &&
        m.swarmStatus?.runId !== PENDING_SWARM_RUN_ID,
    );
    ...
    return () => {
      useSessionStore
        .getState()
        .removeSwarmStatus(sessionId, PENDING_SWARM_RUN_ID);
    };
          ...
          : m.role === "user"
            ? (content: string) => {
                const parsedAttachment =
                  m.attachment ??
                  parseAttachmentPrefix(content)?.attachment ??
                  null;
                const parsedRemaining = m.attachment
                  ? content
                  : (parseAttachmentPrefix(content)?.remaining ?? content);
                if (parsedAttachment) {
                  return (
