...
describe("ToolCallSummary — renders closed segments compactly", () => {
  ...
  it("renders individual rows when ≤4 closed segments", () => {
    const segments = [closed("a"), closed("b"), closed("c"), closed("d")]
  ...
  it("aggregates >4 closed segments into a collapsed summary chip with count breakdown", () => {
    const segments = [
  ...
  it("expands on click to show the individual rows", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /调用 5 个工具/ }))
    expect(screen.getByText("a")).toBeInTheDocument()
    expect(screen.getByText("b")).toBeInTheDocument()
    expect(screen.getByText("c")).toBeInTheDocument()
    expect(screen.getByText("d")).toBeInTheDocument()
  ...
  it("keeps unclosed (in-flight) tool rows visible while collapsed closed tools", () => {
    ...
    expect(screen.getByText(/调用 5 个工具/)).toBeInTheDocument()
  })
  ...
  it("falls back to '工具' label when a closed segment's JSON is malformed", () => {
    const segments: Segment[] = [
      ...
      { type: "tool", content: "fifth", closed: true, start: 41 },
    ]
    render(<ToolCallSummary segments={segments} />)
    // Falls back to 工具 when name extraction fails — that's fine, the summary
    // chip still aggregates to "工具 × 5".
    expect(screen.getByText(/调用 5 个工具/)).toBeInTheDocument()
  })
})
