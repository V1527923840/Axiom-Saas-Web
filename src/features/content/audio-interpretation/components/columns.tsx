...
export const columns: ColumnDef<AudioInterpretationItem>[] = [
  {
    accessorKey: "title",
    header: ({ column }) => (
      <Button
    ...
    cell: ({ row }) => {
      ...
      return (
        <Button
          ...
          onClick={() => {
            window.dispatchEvent(new CustomEvent('open-audio-detail', { detail: item }))
          }}
        >
          查看
        </Button>
...
// Detail Dialog Component for Audio Interpretation
interface AudioInterpretationDetailDialogProps {
  item: AudioInterpretationItem | null
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function AudioInterpretationDetailDialog({ item, open, onOpenChange }: AudioInterpretationDetailDialogProps) {
  if (!item) return null
